...
- stores deterministic embeddings as `vector(16)`;
...
| `embedding` | `vector(16)` | Current deterministic embedding |
...
PostgreSQL entrypoint scripts run only when a database volume is first created. Switching the container image does not rerun initialization against an existing volume.
...
Do not remove a volume that contains data you need. Back it up and apply the idempotent SQL script through `psql` instead.
...
CI performs equivalent checks for the extension, fixed vector dimension, and cosine index.
